...
public class DealBusinessAssignFragment extends BaseFragment
{
    ...
    public void deal()
    {
        ...
        {
            Toast.makeText(getContext(), "请先输入数量!", Toast.LENGTH_LONG).show();
            return;
        }
        String type = "转让";
    ...
    public void getTenInfo(final boolean isNeedReqOther)
    {
        RetrofitUtils.getInstance(getActivity()).api.transaction()
                ...
                .subscribe(new BaseObserver<QuotesData>(getActivity(), false)
                {
                    ...
                    {
                        Log.e("<<<assign ==== ", "assign , getTenInfo");
                        if (resultData.result == 200)
                        {
                            mDataBuy.clear();
                            mDataSale.clear();
    ...
    {
        mEtNum.setHint("转让数量");
        mTvBusiness.setText("转让");
        setBgColor(mTvBusiness, "#CFA32B");
        mTvStore.setText("清仓");
        mLayoutEnterPrice.setVisibility(View.GONE);
        mLayoutAssign.setVisibility(View.VISIBLE);
